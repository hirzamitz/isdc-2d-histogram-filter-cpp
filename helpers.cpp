...
vector< vector<float> > normalize(vector< vector <float> > grid) {
	...
	// Calculate the total of all grid values
	...
	// Normalize each cell in the grid of values
	for (int i = 0; i < grid.size(); i++){
		row.clear();
		for (int j = 0; j < grid[0].size(); j++){
...
vector < vector <float> > blur(vector < vector < float> > grid, float blurring) {
	...
	vector <vector <float> > window(3, vector<float>(3, 0.0));

	// Set the window blurring values for a 3x3 grid
	...
	// Calculate the blurred value for the 3x3 grid based on the window blurring values
